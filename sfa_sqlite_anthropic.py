...
# Run SQLite agent with default compute loops (3)
uv run sfa_sqlite_anthropic.py -d ./data/analytics.db -p "Show me all users with score above 80"

# Run with custom compute loops
uv run sfa_sqlite_anthropic.py -d ./data/analytics.db -p "Show me all users with score above 80" -c 5
...
# Global variables for SQLite connection
DB_PATH = None
connection = None
cursor = None
...
def describe_table(reasoning: str, table_name: str) -> str:
    ...
    try:
        ...
        for col in schema_info:
            formatted_schema.append(f"Column: {col[1]}, Type: {col[2]}, Nullable: {not col[3]}")
...
def sample_table(reasoning: str, table_name: str, row_sample_size: int) -> str:
    ...
    try:
        ...
        # Format the output
        output = []
        output.append(" | ".join(columns))  # Header
        output.append("-" * (len(" | ".join(columns))))  # Separator
        for row in rows:
...
<instructions>
    <instruction>Use the provided tools to explore the SQLite database and construct the perfect query.</instruction>
...
def main():
    ...
    try:
        ...
        console.print(f"[red]Error connecting to SQLite database: {str(e)}[/red]")
        sys.exit(1)
                ...
                if tool_calls:
                    for tool_call in tool_calls:
                        ...
                        try:
                            ...
                            console.print(f"[blue]Tool Call Result:[/blue] {func_name}(...) ->\n{result}")
                            messages.append(
                                {
                                    "role": "user",
                                    "content": [
